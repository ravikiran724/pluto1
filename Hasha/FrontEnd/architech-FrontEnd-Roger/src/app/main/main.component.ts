...
export class MainComponent implements OnInit, OnDestroy {
  ...
  sideBarFilterClass: any = [
    ...
    {
      sideBarSelect: "sidebar-color-6",
      colorSelect: "sidebar-color-green",
    },
    {
      sideBarSelect: "sidebar-color-7",
      colorSelect: "sidebar-color-green",
    },
    {
      sideBarSelect: "sidebar-color-8",
      colorSelect: "sidebar-color-green",
    },
  ];
  ...
  constructor(
    ...
    breadcrumbService.addFriendlyNameForRoute(
      "/user-management/usermanagelist",
      "User List"
    );
    breadcrumbService.addFriendlyNameForRoute(
      "/user-management/usergridlist",
      "User Grid"
    );
  ...
  ngOnInit() {
    ...
    this.coreService.shareKeySearch.subscribe((x) => (this.keySearch = x));
  }
  changeKeySearch(key) {
    this.coreService.changeKeySearch(key);
  }
  ngOnDestroy() {
  ...
  sidebarFilter(selectedFilter) {
    for (var i = 0; i < this.sideBarFilterClass.length; i++) {
      document
        ...
        .classList.remove(this.sideBarFilterClass[i].colorSelect);
      if (
        this.sideBarFilterClass[i].colorSelect == selectedFilter.colorSelect
      ) {
        document
          .getElementById("main-app")
          .classList.add(this.sideBarFilterClass[i].colorSelect);
      }
    }
    document.querySelector(".radius-circle").classList.remove("radius-circle");
    document
      .getElementById(selectedFilter.sideBarSelect)
      .classList.add("radius-circle");
  }
  ...
  headerFilter(selectedFilter) {
    for (var i = 0; i < this.headerFilterClass.length; i++) {
      document
        .getElementById("main-app")
        .classList.remove(this.headerFilterClass[i].colorSelect);
      if (this.headerFilterClass[i].colorSelect == selectedFilter.colorSelect) {
        document
          .getElementById("main-app")
          .classList.add(this.headerFilterClass[i].colorSelect);
      }
    }
    document.querySelector(".radius-active").classList.remove("radius-active");
    document
      .getElementById(selectedFilter.headerSelect)
      .classList.add("radius-active");
  }
  ...
  customizeSidebar() {
    if (
      ...
      if (
        !document
          .getElementById("main-app")
          .classList.contains("sidebar-overlay")
      ) {
      ...
      for (let i = 0; i < main_div.length; i++) {
        if (main_div[i].classList.contains("sidebar-overlay")) {
          document
            .getElementById("main-app")
            .classList.remove("sidebar-overlay");
        }
      }
    }
    //for responsive
